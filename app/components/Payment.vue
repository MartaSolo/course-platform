...
const setupStripe = () => {
  const StripeConstructor = (
    window as Window & { Stripe?: (key: string) => StripeInstance }
  ).Stripe;
  if (StripeConstructor) {
    stripe.value = StripeConstructor(config.public.stripeKey as string);
  }
...
const handleSubmit = async () => {
  ...
  try {
    const response = await stripe.value?.confirmCardPayment(secret as string, {
      payment_method: {
